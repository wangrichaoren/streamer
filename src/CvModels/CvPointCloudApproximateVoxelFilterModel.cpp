...
CvPointCloudApproximateVoxelFilterModel::CvPointCloudApproximateVoxelFilterModel()
    : _box(new QGroupBox())
...
unsigned int CvPointCloudApproximateVoxelFilterModel::nPorts(PortType portType) const
{
...
bool CvPointCloudApproximateVoxelFilterModel::eventFilter(QObject *object, QEvent *event)
{
    if (object == _box) {
        if (_pc->empty()) {
            return false;
...
NodeDataType CvPointCloudApproximateVoxelFilterModel::dataType(PortType const portType,
...
std::shared_ptr<NodeData> CvPointCloudApproximateVoxelFilterModel::outData(PortIndex portIndex)
{
    if (portIndex == 0) {
...
QJsonObject CvPointCloudApproximateVoxelFilterModel::save() const
{
...
void CvPointCloudApproximateVoxelFilterModel::load(const QJsonObject &js)
{
...
void CvPointCloudApproximateVoxelFilterModel::compute()
{
    ...
    approximate_voxel_filter.setInputCloud(d->getData());
    approximate_voxel_filter.setLeafSize(x->text().toFloat(),
                                         y->text().toFloat(),
                                         z->text().toFloat());
    approximate_voxel_filter.filter(*_pc);

    auto after = _pc->size();

    _res = "before size: " + std::to_string(before) + "\nafter size: " + std::to_string(after);
...
void CvPointCloudApproximateVoxelFilterModel::setInData(std::shared_ptr<NodeData> nodeData,
